...
rc_trx50_enum_raid_arrays(void)
{
	...
	// TRX50-specific: Simplified RAID array enumeration (removed aggressive loops that caused lockup)
	rc_printk(RC_DEBUG, "rc_trx50_enum_raid_arrays: TRX50 simplified RAID array enumeration\n");
	{
		rc_printk(RC_INFO, "rc_trx50_enum_raid_arrays: TRX50 enumerating RAID arrays\n");
		
		// Simple SCSI rescan without aggressive loops
		rc_printk(RC_DEBUG, "rc_trx50_enum_raid_arrays: TRX50 simple SCSI rescan\n");
		if (rc_state.host_ptr) {
			scsi_scan_host(rc_state.host_ptr);
		}
	}
